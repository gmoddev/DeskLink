...
| WinUI product shell | PR 8 product preview done | Locked self-contained Windows App SDK/C++/WinRT payload; onboarding, Nearby/manual pairing, authenticated Devices, production monitor authoring, concrete per-PC clipboard/audio intent, gain/mute, fullscreen keep-local behavior, crossing presets, bounded focus/return hotkeys, and exact executable/fullscreen profiles. Alpha remains the normal UI until cutover. |
