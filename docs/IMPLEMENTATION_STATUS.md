...
| Emergency fail-local | Done | First Ctrl+Alt+Pause/Break activation synchronously returns input Local while retaining the trusted session; a second activation within three seconds confirms disconnect |
| Input cleanup contract | Done | Backend callback on failure/release |
| Windows SendInput injector | Done | Built only on Windows; remote-focus targets are rechecked every 50 ms and a higher-integrity or uninspectable Default-desktop foreground releases owned state and fails both peers Local instead of leaving the controller suppressed. Physical Windows 11 validation activated elevated Task Manager after focus was active and observed `remote_focused=false` plus `capture_active=false` within one second. |
| Privileged input / UAC boundary | Development Secure product slice implemented; physical qualification open | Separately signed networkless LocalSystem broker, fixed signed active-session SYSTEM helper, exact protected Program Files/reparse/signer/client checks, administrator-protected per-peer machine/pin grant, and replay-resistant pin/nonce/epoch/revision/sequence/lease envelope. The explicit UI defaults to keeping fail-local protection and labels enablement **I know what I'm doing**. Ordinary packages contain no broker. `Default` supports the bounded input set for elevated apps; `Winlogon` requires foreground `consent.exe` and rejects all key/reconciliation input, allowing only pointer-based manual consent/cancel. An exact broker-reported desktop/helper handoff may pause rejected input for at most 750 ms after Windows returns to `Default`; recovery resumes the same lease, while timeout or any authorization/general injection failure still fails Local. No automatic approval, authentication-secret input, sign-in/lock input, secure-desktop video, network service, identity change, or transport downgrade. Full signed two-PC physical/destructive matrix remains mandatory. |
...
| Low-level suppression gate | Done | Atomic route flag, injected-event pass-through, Ctrl+Alt+Pause/Break immediate Local return with double-activation disconnect confirmation |
...
| Controlled edge roaming | Experimental reciprocal slice | Saving a valid route arms roaming in-place; one validated peer-session owner; independent directional grants; nonce-bound direction tokens; collision-to-Local; local observation never suppresses; authenticated epoch-bound pointer feedback permits a two-sample bidirectional edge return; the configured Return shortcut is intercepted by the capture gate; three intent policies, EDID-only physical-distance landing with strict proportional fallback, bounded outward-intent scoring, cooldown, 1.5-second focus timeout, active route/session invalidation, and lifecycle-gated suppression. Physical Windows 11 signoff remains open. |
...
| Monitor configurator | Product shell integrated | WinUI per-PC physical-size canvas; estimated/offline states; snap proposals remain inactive until accepted; keyboard-accessible full-edge authoring and card movement; Advanced one-way/partial segments; complete-graph validation, Local confirmation when capture is active, atomic save, automatic route activation, and live authenticated-session reload |
...
| Companion lifecycle | Done | Native broker-owned tray and product hotkeys; on-demand single-instance WinUI window fully exits on close; tray Open/Return Local/Exit; ordered fail-local shutdown; atomic preferences; short-lived current-user sign-in bootstrap |
| Product preferences and planner | Runtime integration done | Version-8 bounded preferences retain exact older-version migration, add separate default-off voice route/input/gain/echo policy, default-PTT/explicit-continuous local activation, a local received-voice destination defaulting to communications playback, and a local application-input backend with an exact optional external render endpoint. Schema 7 migrates to the DeskLink-driver backend with no external endpoint. The optional explicit peer address, allowlisted local hotkeys, and at most 32 exact foreground rules remain bounded. Main prefers mDNS and uses the address only when discovery misses; every attempt still requires the exact trusted machine ID and certificate pin. Companion listens/advertises; Flexible conservatively connects when configured. Every plan and reconnect starts Local. |
| Per-user runtime broker | Persistent supervision done | `desklink_runtime.exe` owns the UI-facing current-user endpoint, native tray, allowlisted product hotkeys, device/trust/preferences access, bounded state/topology proxy, coordinated fail-local update shutdown, and one broker-owned `desklink_pair.exe`. It launches fixed siblings without a shell, exposes pause/resume and typed runtime state with a bounded retry countdown, reconciles network/config/process changes, blocks replacement owners when cleanup is uncertain, and retries only explicit post-validation availability failures. Graceful peer cancellation, timeout, idle, unreachable, refused, and close-notify loss retry; protocol/TLS/authentication failures remain terminal. Native suspend/resume handling stops Local and requires a fresh session after wake without clearing user pause or security `ActionRequired`. State reads reconcile the exact child handle before forwarding. The first emergency activation returns Local without stopping the child; a second within three seconds uses the distinct terminal exit and bounded safety marker. Other unknown terminal child exits retain their exact bounded decimal/hex exit code for Diagnostics. |
| WinUI product shell | Beta 1 qualification done | Locked self-contained Windows App SDK/C++/WinRT payload; normal Start menu/post-install entry point and short-lived windowless sign-in bootstrap; fixed-sibling bounded broker startup; on-demand single-instance WinUI settings window that releases its working set on close; onboarding, pairing, Devices, monitor authoring, per-PC clipboard/audio intent, gain/mute, fullscreen behavior, crossing presets, hotkeys, and profiles. Diagnostics exposes the live phase, failure class, exact managed-process exit when present, current input-desktop safety state, last secure-desktop interruption, retry countdown, safe next action, a bounded connection check, and explicit fail-local retry instead of an indefinite unexplained Connecting state. CI rejects DPI/accessibility/live-announcement/theme/UTF-8/baseline drift. The cross-version unsigned Beta lowers only the UI API floor on Windows 10 and requires the complete reviewed OpenSSL runtime graph. Hands-on Narrator/scaling/contrast and a second physical Windows 11 PC remain open. Alpha remains the explicitly labeled diagnostics fallback for one migration release. |
| Broker trust and pairing authority | Automated security boundary | Bounded 64-device enumeration exposes machine/name/grants only; reductions and forget stop the active session after Local/owned-input cleanup before persistence. Additions use a separate 90-second, broker-owned, reject-default local authorization candidate bound to the exact stored machine ID, certificate pin, and current grant snapshot; stale identity/grant state fails closed and pairing trust is never replaced. Pairing remains a separate exclusive fixed-sibling operation with a random 128-bit token. |
...
| Current-user control IPC | Done | Version-15 SID-derived named pipe; explicit one-user DACL; remote rejection; mutual process-SID checks; 512 KiB frame ceiling; bounded typed state/mode/audio/voice-activation/preferences/application-output-status/trust/discovery/pairing/permission-authorization data, including retry delay, managed exit code, durable emergency/input-desktop safety state, and the optional trusted-peer endpoint, plus read-only canonical topology snapshots; malformed, incompatible, and oversized live-pipe requests are dropped and the endpoint recovers |
| LAN DNS-SD/mDNS discovery | Done | Native Windows link-local advertise/browse/resolve; bounded opaque callback registry safely ignores delayed/duplicate completions; strict TXT bounds; deterministic conflict reporting; no automatic trust or pairing. A user-saved address may route only to the exact existing trusted peer when discovery returns no matching record; ambiguous/incompatible records remain fail-closed. |
| Foreground profile foundation | Done | At most 32 exact executable-name rules; optional fullscreen match plus a global fullscreen keep-local policy; emergency/manual/exact-profile/global-fullscreen/default precedence; uninspectable required foreground state fails local |
...
| Current-user Windows installer | Beta 1 qualification done | Fixed LocalAppData install; active-runtime/update/UI gates; product-shell primary shortcuts/post-install launch; exact legacy Alpha startup migration; explicit diagnostic Alpha fallback; exact locked payload and pinned Schannel runtime; HKCU cleanup; byte-preserving repair/rollback/upgrade/uninstall repeated with the same artifact on Windows-latest and Server 2022. The distinct unsigned Beta admits Windows 10 build 19045 only with exact reviewed OpenSSL runtime hashes and prominent experimental labeling. |
| Development Secure installer | Privileged-input package integration implemented; signed install validation open | Dedicated non-exportable RSA-3072 CNG code-signing key; exact self-signed certificate policy; public DER/fingerprint-only export; explicit LocalMachine Root + TrustedPublisher approval; complete executable/uninstaller/Setup RFC 3161 signing and verification; clearly labeled machine-wide Program Files install. The package installs and auto-starts the idle networkless broker but does not grant authority. Its protected exact-peer opt-in is separately elevated and defaults off. Not public production trust. |
| Fail-local Windows update coordinator | PR 9A cutover foundation done | Explicit local packages only; exact hashes; newer-candidate/current-version rollback pair; product-shell signer anchor; ordered Local/runtime/UI shutdown; bounded Setup job; separate shell/broker/state health probes; exact pre-update startup restoration on rollback; product-shell restart; unsigned controls isolated to a non-packaged validation target |
...
| Microphone voice protocol/codec | Automated implementation complete | Protocol v5 datagram-only `VoiceFrame`; separate VoiceSend/VoiceReceive bits; pinned Opus 1.6.1 VOIP encoder; exact 48 kHz mono/20 ms/512-byte bounds; independent stream and sequence state |
| Voice authorization and privacy | Automated implementation complete | Reciprocal acknowledged grants after PeerValidated; current nonce; local-only default PTT or explicit continuous mode plus hard mute/device authority; capture closes on release where applicable, revoke, disconnect, endpoint loss, configuration change, and shutdown; reconnect never restores PTT-down and re-evaluates continuous mode only after fresh admission |
| Voice jitter/render | Automated implementation complete | Initial 40 ms and adaptive 120 ms maximum; bounded reorder/queue/gap rules; Opus FEC then PLC; communications-role renderer; local gain and default-on half-duplex echo guard; no AEC claim |
| Windows microphone backend | Native build complete; physical qualification open | Exact/default communications eCapture selection; event-driven shared-mode normalization; explicit endpoint never falls back; build rejects loopback mechanisms; two-PC PTT/privacy/device/loss/feedback matrix remains required |
| Received-voice output router | Automated implementation complete | One canonical decoded block routes locally to communications playback, a provider-neutral application-input output, or both; no protocol state; independent sink failure; monitor-only gain/echo guard; upstream authority reset stops both. `VoiceApplicationOutput` owns an injectable backend and replaces it only after stopping the previous implementation. |
| Optional virtual microphone | Driver/package implementation complete; certification open | Pinned Simple Audio Sample/WaveRT derivative; stable-property `DeskLink Microphone Feed` to genuine `DeskLink Remote Microphone`; exact 48 kHz mono PCM16; fixed 40/60 ms oldest-drop/silence/flush bridge; fixed UAC installer; source-loop rejection; Microsoft signing and physical zero-mic/Discord qualification remain external gates |
| External virtual-audio cable backend | Automated implementation complete; physical qualification open | Separate local adapter for a user-installed production-signed cable such as VB-CABLE; explicit exact `eRender` endpoint only; no default/alternate fallback; 60 ms stale-drop queue; closes on authorization and lifecycle resets; UI opens the official vendor page but does not bundle or silently install the driver |
...
| Windows 10 opaque CNG prototype | Beta, experimental/unsupported | Explicit built-in OpenSSL 3.5 provider; exportable and mismatched credentials rejected; normal product broker uses one fail-closed OS-based `auto` selection; the separately packaged unsigned Beta remains outside the production support baseline |
...
| Windows pairing control | Done | Five-minute window, native two-PC code prompt, explicit input/audio/voice/topology/clipboard grants; existing trust records are not migrated |
...
- Stream Deck plugin
- PR 9B production-signed installer/update qualification, hands-on clean
  supported-system accessibility/localization/sleep/network validation, and
  destructive-fault matrix (the automated policy/Server 2022 foundation is done)
...
- SYSTEM service owning the whole application; the isolated Development Secure
  broker owns only bounded local privileged-input admission and can never own
  product networking, pairing, trust, capture, clipboard, audio, or UI
- permanent elevation
- virtual HID driver
...
Product UX PR 7 ports that monitor model into the WinUI product shell. It keeps
canvas coordinates presentation-only, exposes an equivalent keyboard path,
requires explicit proposal acceptance, and uses a tested save coordinator that
stops the managed runtime Local before atomic replacement and optional resume.
