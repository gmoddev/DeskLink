...
reciprocal acknowledged `VoiceSend`/`VoiceReceive` grants. System-audio grants
do not authorize voice.
...
Voice sequence numbers are independent from audio and pointer lanes. The voice
receiver admits limited reordering, rejects duplicates/stale streams/excessive
future gaps, and bounds playout to a 40 ms initial and 120 ms maximum adaptive
target. A single loss is recovered with Opus FEC when available and otherwise
Opus PLC.
...
20-byte header contains `DLCT` magic, version `11`, request/response frame type,
...
| 31 | `IdentifyPeerDisplays` | first display ordinal | asks the exact authenticated peer to show bounded local identify overlays |
| 32 | `SetVoiceTransmit` | local boolean | starts/stops PTT microphone capture only after current voice admission; never sent over QUIC |
| 33 | `SetVoiceMuted` | local boolean | sets the local hard microphone mute and synchronously stops active PTT when muting |
...
current per-peer audio gain/mute, voice gain/mute/PTT/privacy/failure state,
focus/capture state, retry attempt, bounded
...
Commands 24-25 and 29-31 are internal child bridges on the same current-user
