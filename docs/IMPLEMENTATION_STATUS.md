...
| Per-user runtime broker | Persistent supervision done | `desklink_runtime.exe` owns the UI-facing current-user endpoint, native tray, allowlisted product hotkeys, device/trust/preferences access, bounded state/topology proxy, coordinated fail-local update shutdown, and one broker-owned `desklink_pair.exe`. It launches fixed siblings without a shell, exposes pause/resume and typed runtime state, reconciles network/config/process changes, blocks replacement owners when cleanup is uncertain, and retries only explicit ordinary availability failures. Native suspend/resume handling stops Local and requires a fresh session after wake without clearing user pause or security `ActionRequired`. |
| WinUI product shell | PR 9B automated qualification foundation done | Locked self-contained Windows App SDK/C++/WinRT payload; normal Start menu/post-install entry point and short-lived windowless sign-in bootstrap; fixed-sibling bounded broker startup; on-demand single-instance settings window that releases the WinUI/XAML working set on close; onboarding, Nearby/manual pairing, authenticated Devices, monitor authoring, per-PC clipboard/audio intent, gain/mute, fullscreen behavior, crossing presets, bounded hotkeys, and exact profiles. CI rejects DPI/accessibility/live-announcement/theme/UTF-8/baseline drift. An explicit Windows 10 Development Alpha build lowers only the UI API floor and requires the full OpenSSL runtime graph. Hands-on Narrator/scaling/contrast remains open. Alpha remains explicitly labeled diagnostics for one migration release. |
...
| Windows 10 opaque CNG prototype | R&D Development Alpha | Explicit built-in OpenSSL 3.5 provider; exportable and mismatched credentials rejected; normal product broker uses one fail-closed OS-based `auto` selection; separately packaged unsigned prerelease remains unsupported/not production-admitted |
