...
| Product preferences and planner | Runtime integration done | Version-7 bounded preferences retain exact older-version migration, add separate default-off voice route/input/gain/echo policy, default-PTT/explicit-continuous local activation, and a local received-voice destination defaulting to communications playback, and preserve the optional explicit address, allowlisted local hotkeys, and at most 32 exact foreground rules. Main prefers mDNS and uses the address only when discovery misses; every attempt still requires the exact trusted machine ID and certificate pin. Companion listens/advertises; Flexible conservatively connects when configured. Every plan and reconnect starts Local. |
...
| Received-voice output router | Automated implementation complete | One canonical decoded block routes locally to communications playback, virtual microphone, or both; no protocol state; independent sink failure; monitor-only gain/echo guard; upstream authority reset stops both |
