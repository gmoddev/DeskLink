...
| UAC secure-input R&D boundary | Hardened Windows 11 access and elevated-window recovery probes passed; product authorization integration blocked | Default-off networkless LocalSystem service and fixed active-session SYSTEM helper; revision `a786563` exact-build Default release and foreground-consent Escape probes passed on build 26200 with pre-action consent verification, a verified `Winlogon`-to-`Default` transition, visual confirmation, one-shot zero-exit service acknowledgement, and no leftover helper. A later bounded probe proved current-user `ShowWindowAsync(SW_MINIMIZE)` cannot minimize elevated Task Manager while the SYSTEM helper can, after requiring the exact visible top-level elevated foreground and verifying `IsIconic`; the lab service/helper were removed afterward. Portable exact grant/pin/nonce/epoch/sequence/lease gate remains unwired. Development Secure now supplies an explicitly trusted, timestamped, protected Program Files application boundary on two approved PCs, but it does not package the lab service/helper. No product IPC, UAC approval, credential handling, lock-screen input, or secure-desktop video. Full install-path/ACL/reparse/signature attestation and protected machine-wide authorization remain mandatory. |
...
| Development Secure installer | Private two-PC signing/install foundation done | Dedicated non-exportable RSA-3072 CNG code-signing key; exact self-signed certificate policy; public DER/fingerprint-only export; explicit LocalMachine Root + TrustedPublisher approval; complete executable/uninstaller/Setup RFC 3161 signing and verification; clearly labeled machine-wide Program Files install. The old per-user build was removed and the signed build/startup path verified on both approved PCs without changing `%LOCALAPPDATA%\DeskLink`. Not public production trust and does not yet install the secure-input service/helper. |
