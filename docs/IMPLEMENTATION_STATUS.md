...
| WinUI product shell | Beta 1 qualification done | Locked self-contained Windows App SDK/C++/WinRT payload; normal Start menu/post-install entry point and short-lived windowless sign-in bootstrap; fixed-sibling bounded broker startup; on-demand single-instance WinUI settings window that releases its working set on close; onboarding, pairing, Devices, monitor authoring, per-PC clipboard/audio intent, gain/mute, fullscreen behavior, crossing presets, hotkeys, and profiles. CI rejects DPI/accessibility/live-announcement/theme/UTF-8/baseline drift. The cross-version unsigned Beta lowers only the UI API floor on Windows 10 and requires the complete reviewed OpenSSL runtime graph. Hands-on Narrator/scaling/contrast and a second physical Windows 11 PC remain open. Alpha remains the explicitly labeled diagnostics fallback for one migration release. |
