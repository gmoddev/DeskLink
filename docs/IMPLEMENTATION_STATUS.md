...
| WinUI product shell | PR 7 product preview done | Locked self-contained Windows App SDK/C++/WinRT payload; real broker state; guided role selection; untrusted Nearby/manual pairing; authenticated Devices list; and production monitor authoring with stable identities, offline displays, Identify, snap proposals, accessible exact authoring, route diagnostics, and fail-local atomic save. Alpha remains the normal UI until cutover. |
