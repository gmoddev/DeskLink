...
| UAC secure-input R&D boundary | Hardened Windows 11 access probe passed; product integration blocked | Default-off networkless LocalSystem service and fixed active-session SYSTEM helper; revision `a786563` exact-build Default release and foreground-consent Escape probes passed on build 26200 with pre-action consent verification, a verified `Winlogon`-to-`Default` transition, visual confirmation, one-shot zero-exit service acknowledgement, and no leftover helper. No-prompt execution failed closed, all 10 native tests passed, the security diff review found no reportable vulnerabilities, and the lab install was removed. Portable exact grant/pin/nonce/epoch/sequence/lease gate remains unwired. No product IPC, packaging, UAC approval, credential handling, lock-screen input, or secure-desktop video. Production signing, full install-path/ACL/reparse/signature attestation, and protected machine-wide authorization remain mandatory. |
