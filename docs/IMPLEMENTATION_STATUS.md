...
| Current-user control IPC | Done | Version-11 SID-derived named pipe; explicit one-user DACL; remote rejection; mutual process-SID checks; 512 KiB frame ceiling; bounded typed state/mode/audio/voice-PTT/preferences/trust/discovery/pairing/permission-authorization data, including retry delay and the optional trusted-peer endpoint, plus read-only canonical topology snapshots; malformed and oversized live-pipe requests are dropped and the endpoint recovers |
