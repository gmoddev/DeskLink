...
| Current-user Windows installer | PR 9A cutover foundation done | Fixed LocalAppData install; active-runtime/update/UI gates; product-shell primary shortcuts/post-install launch; exact legacy Alpha startup migration; explicit diagnostic Alpha fallback; exact locked payload and pinned Schannel runtime; HKCU cleanup; byte-preserving state upgrade/uninstall; unsigned CI artifact only until production signing and clean-system qualification |
...
- Stream Deck plugin
- PR 9B production-signed installer/update qualification, clean supported-system
  accessibility/localization/sleep/network validation, and destructive-fault matrix
